...
interface TaskTableProps {
  ...
  /** Ids marcados. Vazio significa nenhuma seleção em curso. */
  selecionadas: string[];
  /** Marca ou desmarca uma linha. */
  onAlternarSelecao: (taskId: string) => void;
...
export const TaskTable: React.FC<TaskTableProps> = ({
  ...
  const selecionando = marcadas.size > 0;
  const todasMarcadas = tasks.length > 0 && tasks.every((t) => marcadas.has(t.id));
  ...
  return (
    <div className="overflow-x-auto -mx-1 px-1">
      ...
      >
        <thead>
          <tr className="group/head border-b border-slate-200 dark:border-slate-800">
            <th className="w-9 px-2" style={{ width: 36 }}>
              <input
                type="checkbox"
                checked={todasMarcadas}
                onChange={(e) => onSelecionarTodas(e.target.checked)}
                aria-label="Selecionar todas as tarefas desta lista"
                className={`h-3.5 w-3.5 cursor-pointer accent-slate-900 dark:accent-white transition-opacity ${
                  selecionando ? 'opacity-100' : 'opacity-0 group-hover/head:opacity-100 focus:opacity-100'
                }`}
              />
            </th>
            {columns.map((col) => (
            ...
            return (
              <tr
                ...
                className={`group border-b border-slate-100 dark:border-slate-800/70 cursor-pointer transition-colors ${
                  ...
                  marcadas.has(task.id) ? 'bg-slate-100 dark:bg-slate-800/60' : ''
                }`}
              >
                <td className="px-2 py-2.5 align-top" style={{ width: 36 }}>
                  <input
                    type="checkbox"
                    checked={marcadas.has(task.id)}
                    onClick={(e) => e.stopPropagation()}
                    onChange={() => onAlternarSelecao(task.id)}
                    aria-label={`Selecionar ${task.title || 'tarefa'}`}
                    className={`h-3.5 w-3.5 cursor-pointer accent-slate-900 dark:accent-white transition-opacity ${
                      marcadas.has(task.id)
                        ? 'opacity-100'
