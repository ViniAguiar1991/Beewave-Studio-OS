      ...
      <div className="clean-card p-6 md:p-8 space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-slate-200 dark:border-slate-800">
          {[
            ...
            { key: 'prompts', label: '🤖 Prompts Ocultos (IA)', highlight: true },
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-800">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base font-bold font-display text-slate-900 dark:text-white">
                  Prompts Embutidos e Ocultos do Sistema (IA)
                </h2>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-2xl">
                Essas instruções ficam invisíveis para a equipe e definem como o Gemini gera e encadeia cada etapa (Briefing → Headline → Copy / Carrossel / Roteiro → Chat de Ajuste).
              </p>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1.5 rounded-xl border border-slate-300 dark:border-slate-700 px-3 py-2 text-xs font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all cursor-pointer"
              >
              ...
              >
                {promptsSaved ? <Check className="h-3.5 w-3.5 text-emerald-500" /> : <Save className="h-3.5 w-3.5" />}
                <span>{promptsSaved ? 'Prompts Salvos!' : 'Salvar Prompts'}</span>
              </button>
            </div>
          </div>

          <form onSubmit={handleSavePrompts} className="space-y-6 text-xs">
            {/* Prompt 1: Gerador de Headlines */}
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200/80 dark:border-slate-800 space-y-2">
              <div className="flex items-center justify-between">
                <label className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
                  <span className="grid h-5 w-5 place-items-center rounded bg-slate-200 dark:bg-slate-800 text-[10px] font-mono">1</span>
                  <span>Prompt de Headlines (Etapa 1)</span>
                </label>
                <span className="text-[10px] text-slate-400">Gera 3 opções de ganchos virais</span>
              </div>
              <textarea
                rows={3}
                value={promptsForm.headlinePrompt}
                onChange={(e) => setPromptsForm({ ...promptsForm, headlinePrompt: e.target.value })}
                className="clean-input w-full p-3 font-mono text-[11px] leading-relaxed bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700"
                placeholder="Instruções de como o Gemini deve pensar para criar headlines..."
              />
            </div>

            {/* Prompt 2: Legenda / Copy Feed */}
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200/80 dark:border-slate-800 space-y-2">
              <div className="flex items-center justify-between">
                <label className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
                  <span className="grid h-5 w-5 place-items-center rounded bg-slate-200 dark:bg-slate-800 text-[10px] font-mono">2A</span>
                  <span>Prompt de Legenda / Copywriting Padrão (Etapa 2)</span>
                </label>
                <span className="text-[10px] text-slate-400">Leva em conta Briefing + Headline escolhida</span>
              </div>
              <textarea
                rows={3}
                value={promptsForm.copyCaptionPrompt}
                onChange={(e) => setPromptsForm({ ...promptsForm, copyCaptionPrompt: e.target.value })}
                className="clean-input w-full p-3 font-mono text-[11px] leading-relaxed bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700"
                placeholder="Instruções de estrutura de copy, escaneabilidade e CTA..."
              />
            </div>

            {/* Prompt 3: Roteiro de Carrossel */}
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200/80 dark:border-slate-800 space-y-2">
              <div className="flex items-center justify-between">
                <label className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
                  <span className="grid h-5 w-5 place-items-center rounded bg-slate-200 dark:bg-slate-800 text-[10px] font-mono">2B</span>
                  <span>Prompt de Carrossel (Lâminas e Conteúdo Slide a Slide)</span>
                </label>
                <span className="text-[10px] text-slate-400">Estrutura visual de 5 a 7 slides</span>
              </div>
              <textarea
                rows={3}
                value={promptsForm.copyCarouselPrompt}
                onChange={(e) => setPromptsForm({ ...promptsForm, copyCarouselPrompt: e.target.value })}
                className="clean-input w-full p-3 font-mono text-[11px] leading-relaxed bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700"
                placeholder="Instruções para narrativa e quebra de conteúdo nos slides..."
              />
            </div>

            {/* Prompt 4: Roteiro de Vídeo */}
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200/80 dark:border-slate-800 space-y-2">
              <div className="flex items-center justify-between">
                <label className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
                  <span className="grid h-5 w-5 place-items-center rounded bg-slate-200 dark:bg-slate-800 text-[10px] font-mono">2C</span>
                  <span>Prompt de Roteiro de Vídeo (Reels / TikTok / Shorts)</span>
                </label>
                <span className="text-[10px] text-slate-400">Gancho inicial (0-3s), dinâmica e CTA</span>
              </div>
              <textarea
                rows={3}
                value={promptsForm.copyScriptPrompt}
                onChange={(e) => setPromptsForm({ ...promptsForm, copyScriptPrompt: e.target.value })}
                className="clean-input w-full p-3 font-mono text-[11px] leading-relaxed bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700"
                placeholder="Instruções de roteirização com gancho sonoro, visual e desenvolvimento..."
              />
            </div>

            {/* Prompt 5: Chat Refinador */}
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200/80 dark:border-slate-800 space-y-2">
              <div className="flex items-center justify-between">
                <label className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
                  <span className="grid h-5 w-5 place-items-center rounded bg-slate-200 dark:bg-slate-800 text-[10px] font-mono">3</span>
                  <span>Prompt do Chat Copiloto / Refinador de Conteúdo</span>
                </label>
                <span className="text-[10px] text-slate-400">Ajustes pontuais pedidos pelo usuário</span>
              </div>
              <textarea
                rows={3}
                value={promptsForm.chatRefinePrompt}
                onChange={(e) => setPromptsForm({ ...promptsForm, chatRefinePrompt: e.target.value })}
                className="clean-input w-full p-3 font-mono text-[11px] leading-relaxed bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700"
                placeholder="Instruções para o copiloto ajustar e refinar o texto com precisão..."
              />
            </div>

            {/* Prompt 6: Notícias & Tendências */}
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200/80 dark:border-slate-800 space-y-2">
              <div className="flex items-center justify-between">
                <label className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
                  <span className="grid h-5 w-5 place-items-center rounded bg-slate-200 dark:bg-slate-800 text-[10px] font-mono">4</span>
                  <span>Prompt de Curadoria de Notícias & Tendências</span>
                </label>
                <span className="text-[10px] text-slate-400">Busca orientada a nichos e turnos</span>
              </div>
              <textarea
                rows={3}
                value={promptsForm.newsTrendsPrompt}
                onChange={(e) => setPromptsForm({ ...promptsForm, newsTrendsPrompt: e.target.value })}
                className="clean-input w-full p-3 font-mono text-[11px] leading-relaxed bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700"
                placeholder="Instruções para a IA garimpar novidades, ganchos e tendências relevantes..."
              />
            </div>
            ...
            <div className="flex justify-end pt-2">
              <button
                type="submit"
                className="flex items-center gap-2 rounded-xl bg-slate-900 hover:bg-slate-800 dark:bg-white dark:hover:bg-slate-100 text-white dark:text-slate-900 px-6 py-2.5 text-xs font-bold shadow-sm transition-all cursor-pointer"
              >
                {promptsSaved ? <Check className="h-4 w-4 text-emerald-500" /> : <Save className="h-4 w-4" />}
                <span>{promptsSaved ? 'Alterações Salvas com Sucesso!' : 'Salvar Todos os Prompts'}</span>
              </button>
            </div>
          </form>
        </div>
